...
[Since Facebook's aquisition of Mapillary last year](https://blog.mapillary.com/news/2020/06/18/Mapillary-joins-Facebook.html) the team have been making some fairly significant changes to the backend infrastrure of the platform.

Our apps, Map the Paths Web and Map the Paths Uploader, rely heavily on various Mapillary API components.

As some of the documentation is yet to be published and the shutdown of v3 on August 18, 2021 rapidly approaching ([Mapillary API v4 documentation here]()), I wanted to use this weeks post to document our migration path from v3 to v4, should it prove useful to anyone else.

## Autorization

This still follows an Oauth 2.0 flow.
...
The endpoint for the Oauth 2.0 token exhange remains identical between v3 and v4: `https://www.mapillary.com/connect`.
...
We use Mapillary JS for the core navigation between images in a sequence within Map the Paths.
...
The jump for v3 to v4 brings in a lot of new features round 3D content... something we're very interested in exploring in the future.
...
Users can import sequences from Mapillary manually (or via Map the Paths Uploader) into Map the Paths. This allows users to add metadata to the sequences (e.g. type of transport used to capture them) and group them together (e.g. create tours).

The root endpoint for getting images/sequences has changed:
...
It should also be noted that the `images` response for v4 no longer provides user (at time of writing) the URL to download the original image uploaded, providing only a processed 2048 wide image (`thumb_2048_url`).
...
The [Map the Paths Uploader](https://www.mapthepaths.com/uploader) allows you to upload your tours to Mapillary.
...
Whilst this new endpoint is not documented at the time of writing, it's fairly easy to understand using the logic written into [Mapillary Tools](https://github.com/mapillary/mapillary_tools/), Mapillary's own command line uploader.

[The upload process remains unchanged](https://github.com/mapillary/mapillary_tools/blob/a0716c0a5f9ebce451ec1ef362bacb460c4737f9/mapillary_tools/upload_api_v4.py):

1. open upload session
2. start upload using users oauth token
3. close upload session

We will be updating the Map the Paths Uploader to v0.4 to support this change very soon. [Stay tuned for the announcement by signing up for Trek View updates](https://landing.mailerlite.com/webforms/landing/i5h6l6)!
...
* images not imported but exist on Mapillry

Using v3 of the Mapillary API we used a mix of image and sequence endpoint searches to return results in a an area (bounding box, `bbox`).
...
Querying `bbox`es in v3 was very expensive in terms of number of API calls. Vector tiles inv provide a much easier way to visualise the vast amounts of data that is usually returned.
...
We have used Mapillary detections and in Map the Paths Web since its first release.

[I would first recommend taking a look at my post which describes the v3 API endpoints for these functions in detail and also defines the difference between detections and features](/blog/2020/playing-with-mapillary-api).
...
For our use in Map the Paths, we lookup features on a per image basis (we only display features when a user is viewing a single image). However, I should also point out that to search for features in a given area (not image), [the tile endpoint is what you need](https://www.mapillary.com/developer/api-documentation/#point-tiles): `https://tiles.mapillary.com/maps/vtp/mly_map_feature_point/2/{z}/{x}/{y}`.
...
This post only covers our use-cases, and not everything possible via the Mapillary v4 API.

Keep an eye on the Mapillary official documentation, as I understand it, [the team at Facebook are making regular changes and updates to it](https://www.mapillary.com/developer/api-documentation).

## Coming soon to Map the Paths

We're getting ready to ship the changes to bump our code from referencing the Mapillary v3 API to v4.

You won't notice much in the applications, but if you'd like to take a look at the code and how our implementation works check out the open-source Github repositiories for each of them:
...
[Stay tuned for all Trek View product announcements by signing up for our updates](https://landing.mailerlite.com/webforms/landing/i5h6l6)!