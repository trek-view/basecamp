...
Like with yaq, in 360 videos we can account for roll as the whole panoramic view is available to work with, therefore to perform horizon leveling all you need to know the offset of the camera from the ground (0 degrees), or more scientifically put; roll along the `y` axis.
...
It is important to point out that horizon leveling isn't actually horizon leveling. The cameras gyroscope is measuring roll in relation to the `y` axis.

The `y` axis is somewhat of an imaginary measurement, but think of it this way; if the camera was sitting on completely flat ground the roll would be 0. Therefore the top (width ways) would be parallel with the `x` axis.

To level the camera to `y=0` in the 3 examples above, we would need to adjust roll in example one by -10 degrees, the second by -135 degrees, and the third by -340 degrees to make `y=0`.

[As we know from the previous posts](/blog/2022/calculating-heading-of-gopro-video-using-gpmf-part-2), we know that roll (in degrees, like shown above) can be calculated from measurements captured by the gyroscope.
...
It works, but like with yaw, it is a bit shaky, hence a high or low pass filter on the telemetry during calculations could be considered ([see last weeks post for more on that subject](/blog/2022/calculating-heading-of-gopro-video-using-gpmf-part-3)).
...
Like with yaw and pitched, you can offset pitch in a fixed way using ffmpeg, replacing `Y` with the desired pitch offset to account for the mounting error;

```shell
ffmpeg -i <frame_path> -vf v360=input=e:e:pitch=X <out_path>
```

I also tried dynamically leveling pitch in each frames using the same approach as before. Essentially this means there is no concept of up and down. It's very similar to World Lock mode (`z` axis) and horizon lock mode (`y`) axis.
...
And the output with pitch lock:
...
You'll see as camera enters negative pitch, the pitch starts to decrease from 90. It is not hard to account for the camera being upside down. All that is needed is to calculate pitch values using yaw values too (to determine what way up the camera it).
...
The `InitialViewPitchDegrees` determines the starting pitch of the camera. It will default to the center of the image vertically (0 degrees). However, if you know there is a fixed offset you can account for it here.

It is the same concept for `InitialViewRollDegrees`.

The benefit of this method is that no video processing is required.

## Horizon (roll) leveling in photos

GPMF is very verbose. The GoPro sensor data allows us to adjust yaw, pitch and roll for every frame of the video using the data captured.

Many 360 cameras embed a subset of sensor data in photo metadata too.

Unfortunately in the case of GoPro's 360 cameras this is not the case. When shooting in timelapse photo or photo mode non of the sensor data is stored.
...
In next weeks post I'll explore method you can use to estimate camera roll when no sensor data is available.