...
Many people use action cameras for mapping. In many cases, the justification to buy a 360 camera for mapping can't be justified. Action cameras are more versatile.
...
Although we tried to keep our Trek Packs as cheap as possible (>$500), that's still a lot of money.
...
For these trekkers, we also wanted to allow them to brand their imagery in the same way as we will do with equirectangular photos using a logo in the nadir. I've documented how to add a nadir to a 360 equirectangular photo here:
...
<img class="img-fluid" src="/assets/images/blog/2021-01-14/GPAA1088.JPG" alt="Fuerteventura HERO timelapse" title="Fuerteventura HERO timelapse" />

<img class="img-fluid" src="/assets/images/blog/2021-01-14s/trek-view-square-nadir.png" alt="Trek View Nadir" title="Trek View Nadir" />
...
In order to resize the nadir correctly, we first need to determine the size of the photo will will overlay the logo image onto"
...
Let's assume we want to make the watermark 10% of the image height. In which case, we know 10% of the images is 270.4 (2704 * 0.1), so we need to scale the logo file to measure 270 in height (and 270 width, because our logo is square).

Using Imagemagick to do this:
...
You can also play around with other settings like the watermark logos transparency.

For example, we could make it 65% transparent, so that the photo behind is still slightly visible, like so:
...
So we need to place it horizontally (2704-270) 2434, and vertically (2028-270) 1758, which gives us the following command:
...
<img class="img-fluid" src="/assets/images/blog/2021-01-14/GPAA1088-watermark-65transp.JPG" alt="Fuerteventura HERO timelapse with watermark" title="Fuerteventura HERO timelapse with watermark" />

## Step 5: Script it

You'll probably want to run this on multiple images in a timelapse.

Assuming all your photos are the same dimensions and you want the same nadir size overlaid on each, all you need to do is run the final command (step 4) on all your photos.
