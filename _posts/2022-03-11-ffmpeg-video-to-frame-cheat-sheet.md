...
This cheat sheet will help you get to grips with some of the commands that we often use to process videos to make virtual tours.

For this example I'll be using the following equirectangular video (`GS018422.mp4`). 
...
* `-c:a`: sets the audio codec you want to use
	* Options include `aac` for use in combination with H.264 and H.265/HEVC, and copy if you want to preserve the audio codec of the input video
...
```shell
ffmpeg -i IN_VIDEO.mp4 -r 1 fps5/GS018422_%d.png
```

To extract one frame every five seconds:

```shell
ffmpeg -i IN_VIDEO.mp4 -r 1 fps0_2/GS018422_%d.png
```
...
If you download the files you can see there is a quality difference. However, for blog posts (or other low quality uses), the lowest quality `.jpg`) image is more than sufficient given a high enough video resolution (in our case 5.6k). 
...
To extract all frames from between 5 and 14 seconds:
...
To extract all frames from between 5 and 8 seconds, and also between 12 and 15 seconds:
...
First you can use ffmpeg to identify what streams (often referred to as tracks) actually exist:
...
You can see here that the video contains general `Metadata` (not inside a stream/track), `Stream #0:0[0x1]` (stream 0) which is a HEVC video, `Stream #0:1[0x2]` (stream 1) which is AAC audio, and `Stream #0:2[0x3]` (stream 2)
...
Since ffmpeg starts counting from 0, stream 1:1 refers to the audio from input_1.mp4.
...
In this post I added some global metadata: [How to Create a 360 Video from a Timelapse of 360 Images (using ffmpeg)](/blog/2021/turn-360-photos-into-360-video)), but I did not copy over the telemetry track. Let me show you how to do this.

In the example video, we know the telemetry is in stream 2 (`Stream #0:2[0x3](eng)`).

To rebuild a video from frames created earlier using the following command:

```shell
ffmpeg -i GS018422.mp4 -r 1 -q:v 2 jpg2/GS018422_%d.jpg
```

You can rebuild these into a video (with 1 FPS) using the following:
...
* `-framerate 1`: the output framerate per second. Here I want 1 frame every 1 second. If the `-framerate` option is omitted the default will input and output 25 frames per second.
...
2. the newly created video (`GS018422-no-meta.mp4`)
  * here we will only copy the video stream (`Stream #0:0` (you can find this using ffprobe))
...
Input #0, mov,mp4,m4a,3gp,3g2,mj2, from 'GS018422-with-meta.mp4':
  ...
  Duration: 00:00:20.00, start: 0.000000, bitrate: 12122 kb/s
  Stream #0:0[0x1](eng): Data: bin_data (gpmd / 0x646D7067), 86 kb/s (default)
    Metadata:
      handler_name    : GoPro MET
  Stream #0:1[0x2](und): Video: h264 (High) (avc1 / 0x31637661), yuv420p(tv, bt470bg/unknown/unknown, progressive), 5376x2688 [SAR 1:1 DAR 2:1], 12042 kb/s, 1 fps, 1 tbr, 16384 tbn (default)
    Metadata:
...
Again, you'll see ffmpeg complaining about an unsupported codec, this time in stream 0 which is now the telemetry track. Though the telemetry has copied perfectly.
